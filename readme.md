...
`DEVICE_WITHOUT_RECEIPTS`(int) - включение модуля для анализа данных устройств для упаковки без чека(1 - включено, 2 - выключено).<br>
...
На этом этапе мы имеем список всех устройств devices, с экземплярами классов Device, у которого в атрибуте live_event список словарей всех событий(начало упаковки, конец упаковки, уведомления, оповещения, чеки), отсортированные по времени, с которыми мы будем дальше работать. Для оптимизации памяти исходные списки событий у экземпляров классов удаляются(device.alarm_list, device.issue_list, device.suitcases_list, device.receipts_list).
...
В случае, если значение параметра `DEVICE_WITHOUT_RECEIPTS` в config.<span></span>py `1`, то после модуля app.<span></span>py выполняется модуль dwr_service.<span></span>py.
...
Далее вызывается функция `adding_attributes`, которая на вход принимает экземпляр класса Device. Внутри функции в для всех событий типа "упаковка" выставляются свойства для будущих обновлений записей в базе данных последующей логике:
