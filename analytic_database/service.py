    ...
    for device in devices:
        ...
        if device.alarm_list and device.suitcases_list:
            for alarm in device.alarm_list:
                for suitcase in device.suitcases_list:
                    if suitcase.suitcase_start < alarm.alarm_time < \
                            suitcase.suitcase_finish:
        ...
        if device.issue_list and device.suitcases_list:
            for issue in device.issue_list:
                for suitcase in device.suitcases_list:
                    if suitcase.suitcase_start < issue.issue_time < \
                            suitcase.suitcase_finish:
        ...
        if device.line_event:
            ...
            logger.create('Цикл аналитики не был запущен, так как данные для '
...
def sort_receipts(device: Device):
    ...
    for i in range(len(device.line_event)):
        if device.line_event[i]['type'] in {'suitcase_start',
                                            'suitcase_finish'}:
        ...
        if device.line_event[i]['type'] == 'receipt':
            ...
            if last is None:
                j = i
                while j < len(device.line_event) and \
                        device.line_event[j]['type'] not in \
                        {'suitcase_start', 'suitcase_finish'}:
                    j += 1
                if j < len(device.line_event) and \
                        device.line_event[j]['type'] == 'suitcase_finish':
...
def grouping_events(device: Device):
    ...
    while len(device.line_event) > i:
        if i != len(device.line_event) - 1:
            if device.line_event[i]['type'] == 'receipt':
                if len(device.line_event) > i + 1 and \
                        device.line_event[i + 1]['type'] in \
                        {'suitcase_start', 'issue', 'alarm'}:
                    device.line_event.insert(i + 1, {"type": "none"})
            elif len(device.line_event) > i + 1 and \
                    device.line_event[i]['type'] != 'none' and \
                    device.line_event[i + 1]['time'] - \
                    device.line_event[i]['time'] > \
                    timedelta(seconds=TIMEDELTA_CHECK):
                device.line_event.insert(i + 1, {"type": "none",
                                                 "ДРОБЛЕНИЕ ПО ПРИЗНАКУ":
                                                     "ВРЕМЯ"})
        i += 1
...
def add_task(device: Device):
    ...
    for block in device.broken_line_event:
        ...
        package_type_one = package_type_two = quantitypackageone = \
        ...
        for event in block:
            if event['type'] == 'alarm':
                ...
                if event['object'].quantitypackagedouble > 0:
                    quantitypackagedouble += event[
                        'object'].quantitypackagedouble
        ...
        if count_alarm != 0 and count_receipt == count_suitcase_start == 0:
            ...
            block.insert(0,
                         {'task_type': 'чек без упаковки', 'type': 'service'})
            ...
            block.insert(0, {'task_type': 'чеки без упаковок и уведомления',
                             'type': 'service'})
...
def receipt_broken_line_event_sync(device: Device):
    ...
    for block in device.broken_line_event:
        ...
        if block[0]['task_type'] in {'КПУ/КнПУ', 'чек без упаковок',
                                     'смешанные'}:
            for event in block[::-1]:
                ...
                if event['type'] == 'receipt':
                    ...
                    i = len(block) - 1
                    while i >= 0:
                        if block[i]['type'] == 'suitcase_finish' and block[i][
                            'object'].receipt_id == '' and block[i][
                            'object'].package_type == 1 and count_packageone != 0 and \
                                block[i - 1]['type'] == 'suitcase_start':
                        ...
                        elif block[i]['type'] == 'suitcase_finish' and \
                                block[i]['object'].receipt_id == '' and \
                                block[i][
                                    'object'].package_type == 2 and count_packagedouble != 0 and \
                                block[i - 1]['type'] == 'suitcase_start':
                            block[i]['object'].receipt_id = event[
                                'object'].receipt_id
                            ...
                            if count_packageone == count_packagedouble == 0:
                                ...
                                block[i]['object'].receipt_id == '' and \
                                block[i]['object'].package_type in {1, 2, None,
                                                                    'None'} and \
                                block[i - 1][
                                    'type'] == 'suitcase_start' and count_packagedouble == 0 and count_packageone == 0:
                    ...
                    while i >= 0:
                        if block[i]['type'] == 'suitcase_finish' and block[i][
                            ...
                            if count_packageone == count_packagedouble == 0:
                                ...
                                block[i]['object'].package_type in {1, 2, None,
                                                                    'None'} and \
                                block[i - 1]['type'] != 'suitcase_start' and (
                                count_packagedouble == 0 and count_packageone == 0):
                            block[i]['object'].receipt_id = -1
                            if count_packageone == count_packagedouble == 0:
                                break
                        i -= 1
    ...
    for block in device.line_event:
        ...
        for event in block:
            ...
            if event['type'] == 'receipt':
                ...
                i = 0
                while len(block) > i:
                    if len(block) > i + 1 and block[i][
                        'type'] == 'suitcase_start' and block[i][
                        'object'].receipt_id == '' and block[i][
                        'object'].package_type == 1 and count_packageone != 0 \
                            and block[i + 1]['type'] == 'suitcase_finish':
                        block[i]['object'].receipt_id = event[
                            'object'].receipt_id
                        block[i]['object'].package_type_by_receipt = 1
                        count_packageone -= 1
                    ...
                    elif len(block) > i + 1 and block[i][
                        'type'] == 'suitcase_start' and block[i][
                        'object'].receipt_id == '' and block[i][
                        'object'].package_type == 2 and count_packagedouble != 0 \
                            and block[i + 1]['type'] == 'suitcase_finish':
                        block[i]['object'].receipt_id = event[
                            'object'].receipt_id
                        block[i]['object'].package_type_by_receipt = 2
                        ...
                        'type'] == 'suitcase_start' and block[i][
                        'object'].receipt_id == '' and block[i][
                        'object'].package_type in {1, 2, None, 'None'} and \
                            block[i + 1][
                                'type'] == 'suitcase_finish' and \
                            count_packagedouble == 0 and count_packageone == 0:
                        block[i]['object'].receipt_id = -1
                        if count_packageone == count_packagedouble == 0:
                            break
                ...
                while len(block) > i:
                    if len(block) > i + 1 and block[i][
                        ...
                        'object'].receipt_id == '' and block[i][
                        'object'].package_type in {1, 2, None, 'None'} and \
                            block[i + 1]['type'] != 'suitcase_finish' and (
                            count_packagedouble == 0 and count_packageone == 0):
                        block[i]['object'].receipt_id = -1
                        if count_packageone == count_packagedouble == 0:
                            break
...
def adding_attributes(device: Device):
    ...
    def add_template(event):
        ...
        event['object'].issue_attrib['localdate'] = event[
            'object'].suitcase_finish
        ...
        for event in block:
            if event['type'] == 'suitcase_start':
                ...
                # КПУ оплаченная
                    ...
                    elif event['object'].package_type_by_receipt == 2 and \
                            event['object'].package_type == 1:
...
async def update_database(device: Device):
    ...
    for block in device.broken_line_event:
        ...
        for event in block:
            if event['type'] == 'suitcase_start':
                ...
                to_task_event['ord'] += 1
                to_task_event[
                    'parent_id'] = await device.request.create_task_to_event(
                    to_task_event)
    ...
    for block in device.line_event:
        ...
        for event in block:
            if event['type'] in {'suitcase_start', 'issue', 'alarm',
                                 'receipt'}:
        ...
        for event in block:
            ...
            await device.request.update_status(event=event)
        if to_task_event['task_id']:
            await device.request.update_status_and_resolved(
                task_id=to_task_event['task_id'])
...
def create_csv(device: Device):
    ...
    :param device: элемент списка экземпляров активных устройств класса Device.
    """
    with open('suitcases_broken.csv', 'a', encoding='utf-8',
              newline='') as file:
        writer = csv.writer(file, delimiter=";")
        fieldnames = ['status', 'id', 'type', 'date', 'total', 'suitcase',
                      'localdate']
        ...
        for block in device.broken_line_event:
            ...
            for i in block:
                ...
                if i['type'] == 'service':
                    ...
                    writer.writerow((i['type'],
                                     f"ПРЕВЫШЕНИЕ ПО ВРЕМЕНИ МЕЖДУ УПАКОВКАМИ {TIMEDELTA_CHECK}"))

                elif i['type'] == 'suitcase_start':
                    writer.writerow(('', i['time'], 'НАЧАЛО УПАКОВКИ',
                                     f"ТИП УПАКОВКИ: {i['object'].package_type}",
                                     f"номер чека: {i['object'].receipt_id}",
                                     f"тип чека: {i['object'].package_type_by_receipt}",
                                     f"polycom_id = {i['object'].polycom_id}"))
                    if i['object'].issue_attrib.get('id'):
                        writer.writerow(
                            (f"id: {i['object'].issue_attrib['id']}",
                             f"total: {i['object'].issue_attrib['total']}",
                             f"suitcase: {i['object'].issue_attrib['suitcase']}",
                             f"date: {i['object'].issue_attrib['date']}",
                             f"localdate: {i['object'].issue_attrib['localdate']}"))
                        writer.writerow(
                            (f"type: {i['object'].issue_attrib['type']}",))
                    ...
                    writer.writerow(('', i['time'], i['type'],
                                     f"ЧИСЛО ОДИНАРНЫХ УПАКОВОК В ЧЕКЕ: {i['object'].quantitypackageone}",
                                     f"ЧИСЛО ДВОЙНЫХ УПАКОВОК В ЧЕКЕ: {i['object'].quantitypackagedouble}",
                                     f"номер чека {i['object'].receipt_id}"))

                # NOTE: уведомления/оповещения
                elif i['type'] == 'issue':
                    writer.writerow(
                        ('', i['time'], i['type'], i['object'].issue_type))

                elif i['type'] == 'alarm':
                    writer.writerow(
                        ('', i['time'], i['type'], i['object'].alarm_type))
            writer.writerow('')
