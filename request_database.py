    ...
    async def create_task_to_event(self, to_task_event):
        ...
        if conn:
            try:
                ...
                logger.create('Произошла ошибка при создании записи в '
                              'сущности task_to_event', e)
                await conn.close()
                return False
        ...
        if isinstance(event, dict) and conn:
            ...
            if task_id == None and isinstance(event, Alarm):
                try:
                    ...
                    logger.create('Произошла ошибка при обновлении записи '
                                  'в сущности polycommalarm, вложенной '
                                  'в упаковку.', e)
                finally:
                    await conn.close()
                ...
                try:
                    ...
                    logger.create('Произошла ошибка при обновлении записи в '
                                  'сущности polycommissue, вложенной '
                                  'в упаковку.', e)
                finally:
                    await conn.close()
